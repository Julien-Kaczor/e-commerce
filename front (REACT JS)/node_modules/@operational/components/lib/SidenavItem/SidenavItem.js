...
const BaseSidenavItem = styled_1.default("div")(({ theme, compact, isActive, end_ }) => {
    return Object.assign({ display: "flex", padding: `${compact ? 10 : 0}px ${compact ? 0 : theme.space.content}px`, height: compact ? "auto" : 32, cursor: "pointer", width: "100%", alignItems: "center", flexDirection: compact ? "column" : "row", justifyContent: compact ? "center" : "flex-start", whiteSpace: "nowrap", userSelect: "none", fontSize: theme.font.size.body, color: isActive ? theme.color.primary : theme.color.text.lightest, fontWeight: theme.font.weight.regular, boxShadow: isActive && compact ? `2px 0 0 inset ${theme.color.primary}` : "none", marginTop: end_ ? "auto" : 0, alignSelf: end_ ? "flex-end" : "flex-start" }, (end_ ? { "& + .operational-ui__sidenav-item_end": { marginTop: 0 } } : {}), { 
...
const SidenavItem = (_a) => {
    var { to, active, icon, label, compact, compactLabel, end, className, children } = _a, props = __rest(_a, ["to", "active", "icon", "label", "compact", "compactLabel", "end", "className", "children"]);
    const isActive = Boolean(active);
    return (React.createElement(OperationalContext_1.default, null, ctx => (React.createElement(BaseSidenavItem, Object.assign({ as: to ? "a" : undefined }, props, { className: end ? "operational-ui__sidenav-item_end" : "", end_: Boolean(end), compact: compact, href: to, onClick: (ev) => {
            ev.stopPropagation();
            if (props.onClick) {
                props.onClick();
            }
