...
const Container = styled_1.default("div")(({ align, theme, onClick, condensed, width, item }) => (Object.assign({ userSelect: "none", label: "contextmenuitem", width: width || (condensed ? 160 : "100%"), whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", backgroundColor: theme.color.white, lineHeight: `${condensed ? 35 : 44}px`, padding: `0 ${theme.space.content}px`, textAlign: align, display: "flex", alignItems: "center" }, (Boolean(typeof item !== "string" && item.description)
    ...
    }), { borderTop: `1px solid ${theme.color.separators.default}`, "&:last-child": {
        paddingBottom: 2,
    } })));
const Title = styled_1.default("p") `
  font-weight: bold;
...
const ContextMenuItemIcon = props => {
    ...
    if (typeof props.item.icon === "string") {
        return (React.createElement(ContextMenuIcon, { iconlocation_: props.iconLocation, color: props.item.iconColor, left: props.iconLocation === "left" || !props.iconLocation, name: props.item.icon }));
    }
    // If it's an object with a React Element as a property
    return React.createElement(React.Fragment, null, props.item.icon);
};
const ContextMenuItem = props => (React.createElement(Container, Object.assign({}, props, { condensed: props.condensed }),
    (!props.iconLocation || props.iconLocation === "left") && (React.createElement(ContextMenuItemIcon, { iconLocation: props.iconLocation, item: props.item })),
    React.createElement(Content, { value: props.item }),
    props.iconLocation === "right" && React.createElement(ContextMenuItemIcon, { iconLocation: props.iconLocation, item: props.item })));
