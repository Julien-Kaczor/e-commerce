...
class Select extends React.Component {
    ...
    render() {
        const _a = this.props, { color, disabled, naked, value, options, filterable, label, onChange, maxOptions } = _a, props = __rest(_a, ["color", "disabled", "naked", "value", "options", "filterable", "label", "onChange", "maxOptions"]);
        const { open, search } = this.state;
        const selectWithoutLabel = (React.createElement(Container, Object.assign({}, props, { color: color, disabled: Boolean(disabled), naked: Boolean(naked), ref: (containerNode) => (this.containerNode = containerNode), role: "listbox", tabIndex: -2, onClick: () => {
                this.setState(prevState => ({
                    open: !prevState.open,
                }));
            } }),
            React.createElement(DisplayValue, { isPlaceholder: Array.isArray(value) ? value.length === 0 : !value }, this.getDisplayValue()),
            Boolean(options.length) && open && (React.createElement(Options, null,
                filterable && (React.createElement(Select_Filter_1.default, { onChange: (filterValue) => {
                        this.setState({
                            search: filterValue,
                        });
                    } })),
                React.createElement(OptionsList, null, options
                    .filter(option => (option.label || String(option.value)).match(RegExp(search, "i")))
                    .map(option => (React.createElement(Select_Option_1.default, { key: String(option.value), onClick: () => {
